...
class Upgrader {
   ...
   */
  static async upgradeInbound (maConn) {
    let encryptedConn
    let remotePeer
    let upgradedConn
    let Muxer
   ...
   * @returns {Promise<Connection>}
   */
 static async upgradeOutbound (maConn) {
    let remotePeerId
    try {
   ...
   */
  static async _encryptInbound (localPeer, connection, cryptos) {
   ...
   * @returns {CryptoResult} An encrypted connection, remote peer `PeerId` and the protocol of the `Crypto` used
   */
  static async _encryptOutbound (localPeer, connection, remotePeerId, cryptos) {
   ...
   */
  static async _multiplexOutbound (connection, muxers) {
   ...
   * @returns {*} A muxed connection
   */
  static async _multiplexInbound (connection, muxers) {
